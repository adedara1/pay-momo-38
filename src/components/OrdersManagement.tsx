...
import { ChevronRight, Download, Filter, Plus, Mail, Phone } from "lucide-react";

const OrdersManagement = () => {
  const { data: transactions, isLoading } = useQuery({
    queryKey: ["transactions-list"],
    queryFn: async () => {
      console.log("Fetching transactions with customer details...");
          ...
          <TableBody>
            {transactions?.map((transaction) => (
              <TableRow key={transaction.id}>
                ...
                <TableCell>
                  {transaction.customer_name || `${transaction.profiles?.first_name} ${transaction.profiles?.last_name}`}
                </TableCell>
                <TableCell>
